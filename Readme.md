...
  </a>
  <a href="https://github.com/DandaneRida/OncoNutrition/graphs/contributors">
    <img src="https://img.shields.io/github/contributors/DandaneRida/OncoNutrition?style=flat&color=brightgreen&logo=github" alt="GitHub contributors" />
  </a>
</p>
...
Certains antioxydants, notamment les **polyphénols**, jouent un rôle protecteur majeur durant le parcours de soins[cite: 2]. À l'inverse, de nombreux plats peuvent aggraver les troubles digestifs ou la fatigue des patients.
...
Le projet intègre un dossier `scripts` essentiel au traitement des données brutes. Il contient :

* Les scripts de **scraping** pour l'extraction automatique des recettes depuis le site *choumicha.ma*.
* L'application des **facteurs de rétention** nutritionnelle.
* Le calcul précis de la **teneur en polyphénols** pour chaque aliment.
* Le script de remplissage et d'insertion dans la base de données à partir des DataFrames (au format `.csv`).
...
Pour rendre l'application accessible aux utilisateurs finaux :

1. **Base de données :** Migrez votre base locale vers un service cloud (comme Supabase) et configurez l'URL de connexion associée.
2. **Backend :** Hébergez votre API FastAPI sur une plateforme dédiée (comme Vercel).
3. **Frontend :** Dans votre code source Flutter, modifiez la variable `baseUrl` pour qu'elle cible votre API en ligne, puis compilez la version finale de l'application sur un appareil mobile.
...
* **Développeur :** [@DandaneRida](https://www.google.com/search?q=https://github.com/DandaneRida)
...
*📝 **Note :** Il s'agit d'une application développée dans le cadre d'un stage. Les repas suggérés font l'objet d'une analyse par l'IA pour garantir des recommandations adaptées.*
